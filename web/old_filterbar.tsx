...
export function MapFilterBar({ selectedFilters, onToggle }: Props) {
    ...
    const buttons = [
        { type: 'VIDEO' as const, icon: Video, label: '┘┘è╪»┘è┘ê', color: 'bg-purple-600' },
        { type: 'IMAGE' as const, icon: Image, label: '╪╡┘ê╪▒', color: 'bg-pink-600' },
        { type: 'PRODUCT' as const, icon: ShoppingBag, label: '┘à┘╪ز╪ش╪د╪ز', color: 'bg-indigo-600' },
        { type: 'REQUEST' as const, icon: Megaphone, label: '╪╖┘╪ذ╪د╪ز', color: 'bg-orange-600' }
    ]
